...
def test_큐에_여러_건이_쌓여있으면_순서대로_완료처리된다(tmp_path):
    # 평균 생산시간 0 -> 매번 즉시 완료 조건 충족
    register_sample("S-001", "실리콘 웨이퍼-8인치", 0.0, 0.92, 50, data_dir=tmp_path)
    first = approve_order(create_order("S-001", "A고객", 200, data_dir=tmp_path).order_id, data_dir=tmp_path)
    second = approve_order(create_order("S-001", "B고객", 300, data_dir=tmp_path).order_id, data_dir=tmp_path)
    third = approve_order(create_order("S-001", "C고객", 400, data_dir=tmp_path).order_id, data_dir=tmp_path)

    # 1건째: 큐 맨 앞은 first, 완료 처리하면 first만 CONFIRMED로 전환
    queue = get_production_queue(data_dir=tmp_path)
    assert queue[0].order.order_id == first.order_id
    completed_1 = complete_current_production(data_dir=tmp_path)
    assert completed_1.order_id == first.order_id
    assert completed_1.status == OrderStatus.CONFIRMED

    # 2건째: 큐 맨 앞이 second로 넘어오고, 완료 처리하면 second가 CONFIRMED로 전환
    queue = get_production_queue(data_dir=tmp_path)
    assert queue[0].order.order_id == second.order_id
    completed_2 = complete_current_production(data_dir=tmp_path)
    assert completed_2.order_id == second.order_id
    assert completed_2.status == OrderStatus.CONFIRMED

    # 3건째: 큐 맨 앞이 third로 넘어오고, 완료 처리하면 third가 CONFIRMED로 전환, 큐는 빈다
    queue = get_production_queue(data_dir=tmp_path)
    assert queue[0].order.order_id == third.order_id
    completed_3 = complete_current_production(data_dir=tmp_path)
    assert completed_3.order_id == third.order_id
    assert completed_3.status == OrderStatus.CONFIRMED

    assert get_production_queue(data_dir=tmp_path) == []
