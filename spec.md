...
models/             # 데이터 클래스 (Sample, Order, 상태 Enum)
repository/         # 저장소 (모든 CRUD를 JSON 파일에 직접 read/write, 메모리 캐시 없음)
services/           # 도메인 로직 (주문 승인/거절, 생산, 출고, 모니터링 계산)
cli/                # 각 메뉴별 입출력(Console I/O) 처리
data/               # samples.json, orders.json 등 실제 데이터 파일
```

- `models`는 순수 데이터 구조만 가지고 있고 로직을 갖지 않는다.
- `repository`는 데이터의 저장/조회(JSON 파일 read/write 포함)만 담당하고 비즈니스 규칙은 갖지 않는다.
- `services`가 `prd.md`의 기능 요구사항(승인 분기, 생산량 계산, 상태 전이 등)을 실제로 수행한다.
- `cli`는 화면 출력과 사용자 입력만 다루고, 판단 로직은 `services`를 호출한다.

이렇게 나누는 이유는 콘솔 입출력 형식이 바뀌더라도 도메인 로직(`services`)에 영향이 없도록 하기 위함이다.
...
상태 전이는 `prd.md` 4장의 전이도를 그대로 따르며, 전이 로직은 `services` 계층에서만 수행한다 (모델이나 CLI에서 직접 상태를 바꾸지 않는다).
...
- 메모리 캐시 없음: `repository`는 상태를 들고 있지 않는다.
...
각 메뉴는 `cli/` 하위의 개별 모듈이 담당하며, 메인 루프(`main.py`)는 메뉴 번호에 따라 해당 모듈을 호출하는 역할만 한다.

## 6. 에러 처리 원칙

- 잘못된 메뉴 번호, 존재하지 않는 시료/주문 ID 입력 등은 예외를 발생시키지 않고 안내 메시지를 출력한 뒤 해당 메뉴를 다시 보여준다.
- 수량 등 숫자 입력값 검증은 `cli` 계층에서 1차로 수행한다.
