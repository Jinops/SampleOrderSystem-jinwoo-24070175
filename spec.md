...
- **생산은 다 끝나야만 완료로 인정한다(중간에 일부만 완료된 것으로 치지 않는다)**: `total_time`이 전부 경과하기 전까지는 `actual_qty` 중 일부라도 이미 만들어져 사용 가능한 상태라고 간주하지 않는다. 즉 "진행률 72%니까 이미 72%만큼은 재고로 써도 된다" 같은 부분 완료는 없으며, 재고 반영과 상태 전이는 `total_time`이 모두 지난 시점에 한 번에 일어난다.
  - 진행률(progress bar)은 어디까지나 사용자에게 진행 상황을 보여주기 위한 표시일 뿐, 실제 데이터(재고/주문 상태) 반영과는 무관하다.
- 생산 완료 처리 시 (= `total_time`이 모두 지난 시점):
...
- **진행률(progress bar)**: 현재 처리 중인 1건에 한해 표시한다.
  - 생산 시작 시각(큐 맨 앞으로 온 시점)을 기록해두고, 조회 시점까지의 경과 시간을 `total_time`과 비교해 진행률(%) = `min(100, elapsed / total_time * 100)`로 계산한다.
  - 완료 예정 시각 = 생산 시작 시각 + `total_time`.
  - 콘솔에는 텍스트 기반 바 형태로 표시한다 (예: `[■■■■■■■□□□] 72%  완료 예정 09:49`).
  - 대기 중인 나머지 주문에는 progress bar를 표시하지 않고, 예상 완료 시각만 누적 계산해 보여준다.
